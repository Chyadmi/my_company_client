 ...
 */
public class DemandeAugmentationFXMLController implements Initializable {
    ...
    public void envoyerDemande(ActionEvent actionEvent) throws IOException {
        if (testChamps()) {
            int res = demandeFacade.ajouterDemandeAugmentation(sal,pourcentage.getValue(), commentaire.getText());
            if (res == 1) {
                ViewLauncher.forward(actionEvent, "EmployeDemandeFXML.fxml", this.getClass());
            } else {
    ...
    private boolean testChamps() {
        return !(commentaire.getText().equals("") || pourcentage.getValue() == null);
    }
